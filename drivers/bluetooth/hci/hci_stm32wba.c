...
static struct net_buf *treat_evt(uint8_t* data, size_t len) {
...
static struct net_buf *treat_acl(uint8_t *data, size_t len,
				 uint8_t* ext_data, uint16_t ext_len)
{
...
static struct net_buf *treat_iso(uint8_t *data, size_t len,
				 uint8_t* ext_data, uint16_t ext_len)
{
...
static int receive_data(uint8_t *data, uint16_t len,
			 uint8_t* ext_data, uint16_t ext_len)
{
			 ...
			 const uint8_t* ext_data, uint16_t ext_length)
{
	...
	err = receive_data((uint8_t *)data, (size_t)length - 1,
...
static tBleStatus bt_ble_ctlr_init(void)
{
	...
	err = BleStack_Init(&init_params_p);
	if (err < 0) {
		return -EIO;
	}
