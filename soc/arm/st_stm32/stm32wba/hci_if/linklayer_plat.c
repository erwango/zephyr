...
LOG_MODULE_REGISTER(link_layer_plat);

#include "scm.h"

#ifdef CONFIG_STM32WBA_DBG_BLE_LL_PLAT
...
void LINKLAYER_PLAT_GetRNG(uint8_t *ptr_rnd, uint32_t len)
{
	...
	ret = entropy_get_entropy_isr(rng_dev, (char *)ptr_rnd, len, ENTROPY_BUSYWAIT);
	if (ret < 0 ) {
		LOG_ERR("Error: entropy_get_entropy failed: %d", ret);
	}
	LOG_SOC_DBG("n %d, val: %p", len, (void *)ptr_rnd);
}
...
void LINKLAYER_PLAT_TriggerSwLowIT(uint8_t priority)
{
	...
	LOG_SOC_DBG("Priotity: %d", priority);
...
void LINKLAYER_PLAT_EnableSpecificIRQ(uint8_t isr_type)
{
	...
	LOG_SOC_DBG("isr_type: %d", isr_type);
